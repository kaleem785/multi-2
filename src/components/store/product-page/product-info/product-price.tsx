...
const ProductPrice: FC<Props> = ({ sizeId, sizes, isCard, handleChange }) => {
  ...
  if (!sizeId) {
    ...
    const discountedPirces = sizes.map(
    ...
    const minPrice = Math.min(...discountedPirces).toFixed(2);
    const maxPrice = Math.max(...discountedPirces).toFixed(2);

    // if all prices are the same , return a single price ; otherwise ,return range
    const priceDisplay =
      minPrice === maxPrice ? `$${minPrice}` : `$${minPrice} - $${maxPrice}`;
    // if a discount exist when minPirce=maxPrice
    let discount = 0;
  ...
  //Scenario 2: If sizeId passed, show specific size price and quantity
  ...
  return (
    <div>
      ...
      <p className='mt-2 text-xs'>{selectedSize.quantity} pieses</p>
    </div>
  );
};
