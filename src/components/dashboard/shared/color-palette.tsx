...
const ColorPalette: FC<ColorPaletteProps> = ({
  ...
  const handleAddProductColor = (color: string) => {
    ...
    const currrentColorData = colors ?? [];
    const existingColor = currrentColorData.find((c) => color === c.color);
    if (existingColor) return;
    const newColors = currrentColorData.filter((c) => c.color !== "");
