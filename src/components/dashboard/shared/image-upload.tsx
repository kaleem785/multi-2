...
const ImageUplaod: FC<ImageUploadProps> = ({
  disabled,
  onChange,
  onRemove,
  value,
  ...
  if (type === "profile") {
        ...
        <div className='relative rounded-full w-52 h-52 bg-gray-200 border-2 border-white shadow-2xl overflow-hidden'>
          {value.length > 0 && (
            <Image
              ...
              width={300}
              height={300}
              className='w-52 h-52 rounded-full object-cover absolute top-0 left-0 buttom-0 right-0'
            />
          )}
          <CldUploadWidget uploadPreset='vfhmlmze' onSuccess={onUpload}>
            {({ open }) => {
              const onClick = () => {
                open();
              };
    ...
    return (
      ...
      >
        ...
        <CldUploadWidget onSuccess={onUpload} uploadPreset='vfhmlmze'>
          {({ open }) => {
            const onClick = () => {
              open();
            };
